...
ItemEvents.modifyTooltips(event => {
	...
	machineToUpgrade.forEach(function (machine,index) {
		event.add(machine,[
			Text.yellow("Machine can only be upgraded with tier installers.")
		]);
	})*/
	...
	event.add('technicresources:room_upgrader_3', [
		Text.yellow('Warning : Can be used only once')
	])
	event.add('technicresources:room_upgrader_4', [
		Text.yellow('Warning : Can be used only once')
	])
	event.add('technicresources:room_upgrader_5', [
		Text.yellow('Warning : Can be used only once')
	])
